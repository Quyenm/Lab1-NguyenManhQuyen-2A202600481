...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for row in results:
        ...
        prompt_trunc = row['prompt'][:37] + "..." if len(row['prompt']) > 40 else row['prompt']
        gpt4o_trunc = row['gpt4o_response'][:37] + "..." if len(row['gpt4o_response']) > 40 else row['gpt4o_response']
        mini_trunc = row['mini_response'][:37] + "..." if len(row['mini_response']) > 40 else row['mini_response']
 
        line = f"{prompt_trunc:<40} | {gpt4o_trunc:<40} | {mini_trunc:<40} | {row['gpt4o_latency']:<15.2f} | {row['mini_latency']:<15.2f}\n"
        table += line
